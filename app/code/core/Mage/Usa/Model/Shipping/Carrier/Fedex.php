    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    protected $_code = self::CODE;
		...
		/**
		 * Store config key of rate types to use
		 *
		 * @var string
		*/
		protected $_accountRateKey = 'carriers/fedex/account_rate';
		
		/**
		 * account_rate config value
		 *
		 * @var boolean
		*/
		protected $_useAccountRate;
		
		/**
		 * rate type to use
		 * Fedex spec states ACCOUNT or LIST
		 *
		 * @var string
		 */
		protected $_rateType;
		...
		/**
		 * array of LIST rate types
		 *
		 * @var array
		 */
		protected $_listRateTypes = array('RATED_LIST_SHIPMENT', 'RATED_LIST_PACKAGE', 'PAYOR_LIST_SHIPMENT', 'PAYOR_LIST_PACKAGE');

		/**
		 * array of ACCOUNT rate types
		 *
		 * @var array
		 */
		protected $_accountRateTypes = array('RATED_ACCOUNT_SHIPMENT', 'RATED_ACCOUNT_SHIPMENT', 'PAYOR_ACCOUNT_SHIPMENT', 'PAYOR_ACCOUNT_PACKAGE');
    ...
    protected function _createTrackSoapClient()
    {
		...
		/**
		 * return LIST or ACCOUNT rate type depending on account_rate config
		 * Fedex spec states ACCOUNT or LIST
		 * Magento fedex doesn't appear to support ACCOUNT as an object properly, so LIST must always be used
		 * the difference is list includes account rates, but is about twice the data to retrieve/parse
		 * we'll fake it later
		 *
		 * @return string
		 */
		public function rateType() {
			return 'LIST';
			//no sense in this till the object is supported
			if(!$this->_rateType) {
				$this->_rateType = $this->useAccountRate() ? 'ACCOUNT' : 'LIST';
			}
			return $this->_rateType;
		}

		/**
		 * get account_rate config key
		 *
		 * @return boolean
		 */
		public function useAccountRate() {
			if(!$this->_useAccountRate) {
				$this->_useAccountRate = (boolean) $this->getConfigData('account_rate');
			}
			return $this->_seAccountRate;
		}
    ...
    protected function _getQuotes()
    {
        ...
        $ratesRequest = array(
            ...
            'RequestedShipment' => array(
                ...
                'RateRequestTypes' => $this->rateType(),
                'PackageCount'     => '1',
     ...
     * null returned if no list/account rates found
     *
     * @param stdClass $rate
     * @return null|float
     */
        ...
        if (is_object($rate)) {
            ...
            foreach ($rate->RatedShipmentDetails as $ratedShipmentDetail) {
                ...
                $rateTypeAmounts[$rateType] = $netAmount;
						...
						//switch which list to look through
						$useratetypes = $this->_listRateTypes;
            if($this->useAccountRate()) {
            	$useratetypes = $this->_accountRateTypes;
            }

            // walk through our list of types - order is important
            foreach ($useratetypes as $rateType) {
    ...
    public function getCode($type, $code='')
    {
        $codes = array(
            'method' => array(
                ...
                'FEDEX_GROUND'                        => Mage::helper('usa')->__('Ground (Business)'),
                'FIRST_OVERNIGHT'                     => Mage::helper('usa')->__('First Overnight'),
                'GROUND_HOME_DELIVERY'                => Mage::helper('usa')->__('Ground (Home Delivery)'),
